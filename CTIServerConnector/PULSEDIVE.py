...
class PULSEDIVE(SuperConnector):
    ...
    # IBM get CTIPs
    def api_con(self):
        #results by id,can make a loop to take each one, problem when threat not found (tid=2 for example)**
        ...
        #check if the category is malware or botnet(malware), then make malware stix2 object
        if response_json_parsed['category']=='malware' or response_json_parsed['category']=='botnet':
